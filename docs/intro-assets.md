...
All six assets are self-hosted. The introduction has no live map API or metered map dependency. Its optional `bayeux:arrival-seen:v2` localStorage preference stays on the device. Reduced-motion visits bypass animation, direct scene links bypass it, and Home never replays it. Skip/Escape are always available. Unsupported WebGL, context loss, failed imagery or an asset-load deadline bypass the introduction without blocking the viewer. GPU resources, animation frames and timers are released on exit.
